...
// file. Each scenario gets a fresh temp project directory (passed via
// x-opencode-directory) so per-directory state — e.g. the session list — is
// isolated between scenarios and between runs. The directory and its
// symlink-resolved form are normalized to <path>.
func Run(target string) (*result.File, error) {
	f := &result.File{Target: target}
	for _, sc := range Scenarios {
		f.Scenarios = append(f.Scenarios, runScenario(target, sc))
	}
	return f, nil
}

func runScenario(target string, sc Scenario) result.Scenario {
