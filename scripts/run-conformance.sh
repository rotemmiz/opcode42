...
#                                              #   forged, both prompting a real model
...
# The provider key must NOT live in the repo: load it from ~/.forge/conformance.env
...
LIVE_ENV="${FORGE_CONFORMANCE_ENV:-$HOME/.forge/conformance.env}"
...
# run_forge_suite <test> <out.json>: build + start a pristine, auth-enabled
# forged on FORGE_PORT, run the given suite test against it, stop it. Forwards the
# provider key the same way as opencode.
run_forge_suite() {
  local test="$1" out="$2"
  local home; home="$(mktemp -d)"
  local bin="$home/forged"
  ( cd "$REPO_ROOT" && go build -o "$bin" ./cmd/forged )
  HOME="$home" OPENCODE_SERVER_USERNAME="$OC_USER" OPENCODE_SERVER_PASSWORD="$OC_PASS" \
...
case "$MODE" in
  self)
    ...
    ;;
  dual)
    FORGE_URL="${2:-http://127.0.0.1:$FORGE_PORT}"
  ...
  live)
    ...
    echo "live dual-run: opencode (truth) vs forge (real model: google/gemini-2.5-flash)"
    run_opencode_suite TestLiveSuite "$RESULTS/live-opencode.json"
    run_forge_suite TestLiveSuite "$RESULTS/live-forge.json"
    echo "=== live diff ==="
    go run "$REPO_ROOT/conformance/cmd/diff" \
      -divergences "$REPO_ROOT/conformance/known-divergences-live.json" \
      "$RESULTS/live-opencode.json" "$RESULTS/live-forge.json"
    ;;
  *)
    echo "usage: $0 [self|dual <forgeURL>|live|live-self]" >&2
    exit 2
    ;;
esac
