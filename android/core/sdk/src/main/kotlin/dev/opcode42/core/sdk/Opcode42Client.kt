...
class Opcode42Client @Inject constructor(
    ...
    suspend fun findFiles(query: String, directory: String? = null, limit: Int = 20): List<String> =
        withContext(Dispatchers.IO) {
            ...
            httpClient.newCall(req).execute().use { resp ->
                ...
                val arr = Opcode42Json.parseToJsonElement(body) as? JsonArray ?: return@use emptyList()
    ...
    suspend fun getSessionDiff(
        ...
        httpClient.newCall(req).execute().use { resp ->
            ...
            val arr = Opcode42Json.parseToJsonElement(body) as? JsonArray ?: return@use emptyList()
            arr.map { Opcode42Json.decodeFromJsonElement(SnapshotFileDiff.serializer(), it) }
        }
    }
    ...
    private suspend fun <T> get(
        ...
        httpClient.newCall(req).execute().use { resp ->
            ...
            parse(Opcode42Json.parseToJsonElement(body))
        }
    }
    ...
    private suspend fun <T> post(
        ...
        httpClient.newCall(req).execute().use { resp ->
            ...
            val elem = try { Opcode42Json.parseToJsonElement(respBody) } catch (_: Exception) { JsonObject(emptyMap()) }
            parse(elem)
        }
    }
